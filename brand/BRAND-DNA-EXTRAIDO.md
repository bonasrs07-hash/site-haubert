# HAUBERT + CASA, DNA extraído do deck "SOCIAL DNA" (ago/2025)

Fonte: `HAUBERT.zip` → 13 pranchas (boards 001–016 + capa + perfis).
Extração de cores feita por amostragem de pixel nos swatches originais (2949×1868).

## 1. A casa

> **"Duas culturas. Uma casa."** / *Two cultures. One house.*
> **"Uma essência. Dois conceitos. Uma conexão que fica."**

Duas operações no MESMO espaço físico, em Novo Hamburgo / RS:

| | CASA COFFEE COLAB | HAUBERT STEAK & GRILLHOUSE |
|---|---|---|
| Mote | **Luz. Leveza. Encontro.** | **Fogo. Força. Tradição.** |
| Turno | Dia | Noite (a partir das 19h) |
| Categoria | Coffee shop / urban culture house | Steakhouse autoral |
| Instagram | @casa.coffee, 1.482 posts · 31,2K seg. | @haubert.steakhouse, 642 posts · 10,8K seg. |
| Tom de voz | Próximo, humano, moderno, convidativo | Confiante, direto, maduro, apaixonado pelo que faz |
| Estética | Claro, natural, urbano, daylight. Acolhedor e leve. | Escuro, quente, intenso, noturno. Robusto e autoral. |

**A regra que une:** *"After 7, CASA é HAUBERT."*, mesma casa, nova atmosfera.
*"Same house. Different energy."*

## 2. Frases proprietárias (usar como copy real do site)

- `DON'T SELL THE PLACE. MAKE PEOPLE WANT TO BELONG.` ← headline-mãe
- `O dia alimenta. A lenha transforma. A casa conecta. A memória fica.`
- `Da luz ao som. Da conversa ao brinde. Da mesa à memória.`
- `Não é só comida. É o motivo do encontro. É o que fica na memória.`
- `Fire is not the show. Fire is the language. We speak it.`
- `Pessoas criam lugares. Lugares criam histórias. Histórias criam comunidade.`
- `Verdade aquece. Verdade conecta. Verdade fica.`
- `Do primeiro café ao último brinde.`
- `Mais café. Mais fogo. Somos cultura.`
- `Algo está mudando.`
- `Support Your Local Everything.` (neon da casa)
- Fecho: `Cozinhamos com propósito. Servimos com verdade. Conectamos pessoas. Criamos memórias. Deixamos legado.`

## 3. Pilares (board 001)

Autenticidade · Conexão · Experiência · Design & Atmosfera · Hospitalidade · Cultura

**Compromisso:** *"Criar uma comunidade real, onde bons momentos viram memórias e memórias viram parte de quem somos."*

## 4. Tipografia (declarada no deck)

- **Títulos:** BEBAS NEUE / **DRUK** (Druk Cond nas pranchas de steak)
- **Texto:** **Space Grotesk** / Neue Haas Grotesk
- Micro-labels e legendas do deck: monoespaçada, caixa alta, tracking largo

## 5. Paletas (hex amostrado do original)

### PALETA CASA, "claro, natural, urbano, daylight"
| Hex | Nome sugerido |
|---|---|
| `#DACFBB` | Areia / linho |
| `#A0A083` | Sage |
| `#555123` | Oliva escuro |
| `#232322` | Tinta (quase preto) |
| `#BE6A44` | Terracota |

### PALETA HAUBERT, "escuro, quente, intenso, noturno"
| Hex | Nome sugerido |
|---|---|
| `#131212` | Carvão |
| `#212120` | Grafite |
| `#631F15` | Brasa (bordô) |
| `#784920` | Caramelo queimado |
| `#D0BDA1` | Areia quente |

### Neutros e acento do sistema
| Hex | Uso |
|---|---|
| `#F6EFE7` | Fundo osso / cream (papel do deck) |
| `#F1EEEA` | Cream alternativo |
| `#A0361F` | **Acento tijolo/ember**, headlines de destaque e links (modal do deck) |

## 6. Estética visual (board 002)

Luz natural · Cores reais (tons quentes, terrosos, neutros, nada forçado) ·
Enquadramentos simples e cinematográficos (menos pose, mais vida) ·
Foco no detalhe (texturas, experiências, comida, bebida, mãos, olhares, fogo) ·
Pessoas reais (clientes, equipe, parceiros, amigos).

**Regra de ouro do feed:** *"Se não representa quem somos, não entra no feed. Verdade gera conexão. Conexão gera comunidade."*

## 7. Conteúdo / eixos de narrativa (viram seções do site)

Fire (técnica, fogo, processo) · Steak (produto e desejo, Ancho, Rib Eye,
T-Bone, Picanha, Filé Mignon, Brisket) · People (o time que faz a casa) ·
Culture (música, arte, cidade) · Table (experiência, encontro, comunidade) ·
Night (a transformação depois das 19h) · Eventos/DJs/Collabs
(In The Flow, Resenha, Bons Tempos, Matcha Club) · Campanha 18.09
(comida que transforma / projetos apoiados: educação, meio ambiente, comunidade).

## 8. O que o ZIP NÃO tem (pendências de asset)

- [ ] Logos vetoriais (CASA e HAUBERT), hoje só existem rasterizados nas pranchas
- [ ] Arquivos de fonte (Bebas Neue e Space Grotesk são gratuitas; **Druk é paga**)
- [ ] Banco de fotos em alta (as pranchas têm thumbs pequenos)
- [ ] Endereço completo, telefone, horário oficial, links de reserva
- [ ] Cardápio real (preços, cortes, drinks)
