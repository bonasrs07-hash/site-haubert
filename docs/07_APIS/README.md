# 07 — APIS · CASA + HAUBERT — site oficial da casa

> Contrato de APIs: endpoints, autenticação, error-handling, versioning.

**Status: sem API própria na Fase 1** — decisão registrada no [ADR-001](../08_DECISOES/adr-001-stack-astro-supabase.md). O contrato que existe hoje é a **camada de serviços** (`src/lib/`), documentada em [`docs/01_ARQUITETURA`](../01_ARQUITETURA/README.md), falando direto com o Supabase sob RLS. Os primeiros endpoints de servidor aparecem na Fase 3 (`POST /api/reserva`) — quando existirem, o contrato deles é escrito aqui **antes** de ser codado.
